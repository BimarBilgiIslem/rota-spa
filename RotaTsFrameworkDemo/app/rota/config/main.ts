...
if (typeof window !== 'undefined') {
     ...
     */
    const setTextPlugin = (constants: IConstants, culture: string, oidc?: IOidcManager) => {
        let redirecting;
        require.config({
            config: {
                text: {
     ...
     * Load framework
     */
    const loadFr = (constants: IConstants) => {
        require(['config/vendor.index'], (): void => {
            //load startup along with rota fr
            require([env.startupModuleName || constants.DEFAULT_STARTUP_MODULE_NAME], (app): void => {
                //validate app
     ...
     * @param oidc OIDC manager
     */
    const init = (constants: IConstants, currentCulture: string, oidc?: IOidcManager) => {
        //settings
        initCulture(currentCulture);
        setTextPlugin(constants, currentCulture, oidc);
        setCrossOriginPaths();
        setRequireConfig();
        setConfig();
        //init fr
     ...
     * @param oidc OIDC manager
     */
    const authCallback = (constants: IConstants, oidc: IOidcManager) => {
        ...
        result.then(user => {
            if (user !== null) {
                ...
                init(constants, currentCulture, oidc);
            }
        },
     ...
     * @param constants Constants
     */
    const anonymousCallback = (constants: IConstants) => {
        const currentCulture = getCulture(constants);
        init(constants, currentCulture);
    }
    //Winter is coming
